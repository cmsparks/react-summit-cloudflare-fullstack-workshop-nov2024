...
- Implement and test `generateAndSaveCard`
  ...
  - You can store the card image data however you prefer, but we stored it as an `arrayBuffer` in R2
    - You'll need to convert the `ReadableStream<Uint8Array>` of an unknown length to a statically sized `Uint8Array` if you want to store the data in r2 as an `arrayBuffer` type
  - Here's some usful documentation on [storing data in R2](https://developers.cloudflare.com/r2/api/workers/workers-api-reference/) and [storing data in KV](https://developers.cloudflare.com/kv/api/write-key-value-pairs/)
